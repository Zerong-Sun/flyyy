...
- C 城模板字段豁免字数下限（资料不足兜底），但**禁止用占位句重复灌水**（如重复 N 遍「更多细节…」填充长度）；字段须给出 1 句真实事实槽位 + 至多 1 次占位提示。现有 400 座 C 城的历史灌水为 v0.1 模板遗留，列入待办逐步重生成。
...
- [x] L2 城市 ≥500，七字段达字数下限；`source_ids` / `content_confidence` 齐全  
- [x] L1 机场可旅行；无航班机场走 PRD §9.2 警告流  
- [x] 商品数量与继承规则满足 §2.4；无禁售类；无纪念品主库  
- [x] `product_market_tags` 覆盖每个 origin\|product 与全部 L2 目的城分类互斥且并集完整（相对当前世界城市集）  
- [x] 单中转联程可检索、可购、可飞完并结算  
- [x] 数据来源页更新城市/商品规模说明；ODbL 等义务仍完整  
- [x] `coverage_report` 由 `test_v2_coverage.py` 门禁（缺文案/缺商品阈值可配置；跑全量 pytest 即执行）
...
**数量：** 每个 L2 城市 **1** 张主头图。v0.2 全量交付采用「区域通用底板 + 色调」过渡方案覆盖非 Demo 的 480 城（1280×720，`gen_city_plates.py`，已写明 attribution 为临时通用资产、后续版本逐步替换为城特定插画）；该方案**不用于 Demo 20 城**（Demo 20 城保留 640×360 原创 AI 插画）。
...
- [x] ≥500 城头图绑定且无水印；许可进 attribution  
- [x] 每个可交易 `product_id` 有独特 64 图标；抽检无品牌包装  
- [x] 成就图标 locked/unlocked 可辨  
- [x] 情报/笔记/hot/cold 图标与 Theme 色板一致  
- [x] L2/W2 反馈资产齐全；减少动态时不崩溃  
- [x] 命名符合 CAS `{domain}_{object}_{variant}_{size}`；无中文文件名  
...
- [x] 三首 BGM + 五条反馈 SFX 文件与 Manifest 一一对应  
- [x] 场景切换无爆音；长时间市场界面不疲劳  
- [x] 静音与分轨音量仍生效  
- [x] 许可写入数据来源页  
...
|----------------|-----|----------|
| ≥ 1.40 | hot | 日噪与品质折扣下仍大概率盈利（ETL 抽检 ≥85% @Q1.0） |
| 1.10–1.39 | normal | 方向对，但噪声和品质可能翻车 |
| &lt; 1.10 | cold | 大概率亏 |
...
- [x] hot/normal/cold 与 ETL 阈值一致；标签随购票目的地更新  
- [x] $200 预测扣款与区间显示正确；无票时拒绝并提示  
- [x] 笔记 Tab 按城汇总；存读档后记录仍在  
- [x] 四类事件概率与互斥规则手测可复现（同种子）  
- [x] 五级反馈阈值可测；减少动态可用  
- [x] SpinBox/快捷数量；&lt;2h 灰显；默认聚焦 ≥2h  
- [x] 无赌场式音效与违规文案  
...
- [x] §6.3 最低集全部可解锁且可回归（单测或脚本模拟统计）  
- [x] 图标文件与 locked 态齐全（30 枚已交付；引用完整性由 `test_achievement_icon_refs_resolve_to_files` 门禁，新增成就必须附真图）
- [~] 20 条 v0.3 时代成就的专用图标未交付（`test_content_assets.ACH_ICON_EXCEPTIONS` 豁免；运行时走 `IconFactory.get_achievement_icon` 类别 fallback 徽章，待后续版本补专属图标）  
- [x] 存读档进度不丢  
- [x] 解锁提示不阻断时间规则（除设置暂停）  
...
| 批次 | 范围 | 数量 |
|------|------|------|
| P0 | Demo 20 枢纽 + 高优先级 L2 城 | ≥30（已交付） |
| P1 | 其余 L2 城（区域通用底板 + 色调，`gen_city_plates.py`，已写明临时通用） | 480 城全量（已交付） |
| 占位 | 颜色底板 + 城市名 overlay | 无（未使用，P1 底板已覆盖全部非 Demo 城） |
...
| 模块 | 关键文件 |
|------|----------|
| 数据层 | `DataService` 加载 `product_market_tags`/`transfer_edges`；`AppState.stats` 存档 |
| 继承标签 | 市场行显示「区域特产/国家标准品」 |
| 联程消费 | `FlightSearch.search_connections` + 两段购票/过场 |
| 航线推荐 | 航班面板顶部热卖目的地推荐 |
| 成就系统 | `AchievementSystem` + `achievements.json`（31 条）+ 成就墙 Tab |
| BGM/设置 | Tab 切换 BGM；夜时变奏；减少动态 |
| 验证 | pytest 65 过；`demo_smoke_logic` OK；SmokeContent OK |

### 7.3 总验收清单（发布门禁）

**内容**

- [x] §2.7 全部勾选（500 城、source_ids/confidence 齐全、商品继承、market_tags、联程、coverage 门禁 — `test_v2_coverage.py`）

**表现**

- [x] §3.6、§4.3 全部勾选（500 城头图 + attribution、商品/成就/反馈图标、3 BGM + 5 SFX 与 Manifest 对应）

**系统**

- [x] §5.8、§6.6 验收通过（锚点/情报/笔记/事件/五级反馈/数量 UX；成就最低集可回归；§6.6 遗留项 20 条成就专用图标已如实标注，走类别 fallback）

**回归**

- [x] Demo §27 自动化（pytest + `demo_smoke_logic` + SmokeContent）仍通过
- [x] 20 枢纽金样航线/贸易无回归（`test_acceptance_demo.py` + `demo_smoke_logic` OK）
- [x] 1080p 地球漫游性能目标（标签/航线上限策略生效）
- [x] 强制声明仍出现在底栏、购票、来源页、启动/菜单  
...
| v0.3 | 2026-07-27 | MR4 已交付：锚点消费、情报/笔记/事件/五级反馈（既有）、联程消费、航线推荐、成就墙、BGM 切换 — pytest 65 + SmokeContent 通过。 |
| v0.4 | 2026-08-02 | 全量完成：80 首都/枢纽城文案重写（A 置信度）、`source_ids` 导出、480 城程序化头图（区域底板+色调，`gen_city_plates.py`）、L2/W2 商人头像（`gen_portraits.py` + PopupEvent 接入）、反馈文案 i18n 化（`sell_console_*`/`celebration_w*`/`sell_result_*`）、SmokeContent 与资产测试扩展 — pytest 82 + SmokeContent + demo_smoke_logic + 内容抽检全过，§7.3 门禁勾齐。 |
| v0.5 | 2026-08-02 | 六维验证修复：§3.1 头图约束更新为「480 城全量区域底板（明确标临时通用，不用于 Demo 20）」；§6.6 如实标注 20 条成就专用图标遗留；§2.3 明确 C 城模板豁免字数下限 + 禁占位灌水（400 C 城灌水列为待办）；成就 icon 引用对齐 10 条（数据层）、新增引用完整性测试门禁；`requirements-art-gen.txt` 补 numpy；`sample_content_review.py` 字数下限对齐 REQ §2.3、C 城豁免、`source_ids` 检查更新；清理 `gen_portraits.py` 死代码 — 全量回归通过。 |
